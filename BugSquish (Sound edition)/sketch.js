...
let spriteSheetFilenames = ["AntBoi.png"];
let spriteSheets = [];
...
function preload() {
  ...
  themeSong = loadSound('assets/BugTheme.mp3');
  squishSound = loadSound('assets/BugSquish.mp3');
  missSound = loadSound('assets/BugLaugh.mp3');
  gameOverSound = loadSound('assets/GameOver.mp3');
  skittering = loadSound('assets/Skittering.mp3');
}
...
function setup() {
  ...
  for(let i=0; i < totalAnimations; i++) {
    animations[i] = new WalkingAnimation(random(spriteSheets),64,64,random(100,300),random(100,300),5,random(0.5,1),5,random([0,1]));
  }
}
...
function draw() {
  switch(game.state) {
    ...
    if (!skittering.isPlaying()) {
      skittering.loop();
    }
      if (!themeSong.isPlaying()) {
        themeSong.loop();
      }
      themeSong.rate(1 + (game.score * 0.05));
